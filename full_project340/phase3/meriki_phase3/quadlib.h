...
expr * tmp_expr;
expr* tempQuadNo; /* krataei to label tis teleutaias OR*/
expr* tempQuadNo2;
expr* ORQuadNo = (expr *) 0;
expr* ANDQuadNo = (expr *) 0;
...
symrec_t * newtemp(){
    ...
    if(sym == NULL){
        return insert(scope, 0, LOCALVAR, name);
    }else{
        return sym;
    }
...
expr * makeTrueList(unsigned label){
    expr * list = newexpr_constnum(label+2);
    list->label = label;
    return list;
}

expr * makeFalseList(unsigned label){
    expr * list = newexpr_constnum(label);
    list->label = label;
    return list;
}

expr * emitPartialRelop(expr * E, expr * arg1, expr * arg2, iopcode op, int line){
    E->truelist = makeTrueList(nextquadlabel()); 
    E->falselist= makeFalseList(nextquadlabel()+1); 
    emit(op, E->truelist, arg1, arg2, currQuad, line);
    emit(jump, E->falselist, (expr*) 0, (expr*) 0, currQuad, line);
    return E;
}

expr * mergePartial(expr * list1, expr * list2){
    expr * head = list1;
    if(head == NULL){
        return list2;
    }
...
void emitBoolConversion(expr * arg1, int line){
    if(arg1->type != boolexpr_e){
        arg1->truelist = makeTrueList(nextquadlabel()); 
        arg1->falselist= makeFalseList(nextquadlabel()+1); 
        emit(if_eq, arg1->truelist, arg1, newexpr_constbool('1'), currQuad, line);
        emit(jump, arg1->falselist, (expr*) 0, (expr*) 0, currQuad, line);
    }
}

void FINALemits(expr * e, int line){
    if(makeFinalEmits){	
        backpatch(e->falselist, nextquadlabel()+2); 
        //backpatch(e->truelist, nextquadlabel());          
		emit(assign, e, newexpr_constbool(1), (expr*) 0, currQuad, line);
    	emit(jump, newexpr_constnum(nextquadlabel()+2), (expr*) 0, (expr*) 0, currQuad, line);
    	emit(assign, e, newexpr_constbool(0), (expr*) 0, currQuad, line);
		makeFinalEmits = 0;
	}  
}

expr* cloneList(expr* head){
    expr* current=head;
    expr * newList = NULL;
    expr* temp=NULL;
