...
expr* reverseList(expr* head){
    ...
    while(current != NULL){
        if(newList == NULL){
            ...
            newList->sym = current->sym;
            newList->type = current->type;
            newList->numConst = current->numConst;
            newList->boolConst = current->boolConst;
            newList->charConst = current->charConst;
            newList->next = NULL;
            temp=newList;
        }else{
            temp1 = (expr*)malloc(sizeof(struct expr));
            temp1->sym = current->sym;
            temp1->type = current->type;
            temp1->numConst = current->numConst;
            temp1->boolConst = current->boolConst;
            temp1->charConst = current->charConst;
            temp1->next = temp;
            temp=temp1;
        }
...
expr * make_call(expr* lvalue, expr* elist, int line){
    expr * list = elist->next;
    expr * new_list = NULL;
    expr* func = emit_iftableitem(lvalue);
    expr* result;
    new_list = reverseList(list);
    while(new_list != NULL){
        emit(param, new_list, (expr*) 0, (expr*) 0, currQuad, line);
